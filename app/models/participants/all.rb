  ...
  class All
    ...
    def add_country_sheet(workbook, country)
      workbook.add_worksheet(name: country) do |sheet|
        ...
        participants = Participant.includes(:survey_responses).eligible_completed_main_block.where(country: country)
    ...
    def country_header
      ['Database ID', 'Email', 'Phone Number', 'Self Generated ID', 'Date of Enrollment',
       ...
       'Network', 'Education', 'Age', 'Can Contact']
    end
    ...
    def participant_row(participant)
      [participant.id, participant.email, participant.pretty_phone_number,
       ...
       participant.baseline&.sgm_network_size, participant.baseline&.education,
       participant.age, participant.baseline&.can_contact]
    end
