...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {
          key: 'participant_count_with_duplicates',
          label: 'Parts w/ Dups',
          sortable: true,
        },
        {
          key: 'average_baselines_for_participants_with_duplicates',
          label: 'Avg Dups / Parts w/ Dups',
          sortable: true,
        },
        {
        ...
        {
          key: 'eligible_participant_count',
          label: 'Eligible Participants',
          sortable: true,
        },
        {
          key: 'completed_participant_count',
          label: 'Participants With Completed Baselines',
          sortable: true,
        },
      ],
    };
  ...
  computed: {
    compMobilizers() {
        ...
        return {
          ...
          _cellVariants: {
            ...
            average_baselines_for_participants_with_duplicates:
              mobilizer.average_baselines_for_participants_with_duplicates >= 2
                ? 'danger'
                : null,
            ip_address_count:
              mobilizer.ip_address_count < mobilizer.participant_count
                ? 'danger'
                : null,
            participant_count_with_duplicates:
              mobilizer.participant_count_with_duplicates > 0 ? 'danger' : null,
            completed_participant_count: this.getColor(
              mobilizer.completed_participant_count
            ),
            self_gen_id_count:
              mobilizer.self_gen_id_count != mobilizer.participant_count
                ? 'danger'
                : null,
          },
        };
      ...
      items.push({
        ...
        eligible_participant_count: items.reduce(
          (a, b) => a + b.eligible_participant_count,
          0
        ),
        completed_participant_count: items.reduce(
          (a, b) => a + b.completed_participant_count,
          0
        ),
        _rowVariant: 'info',
      });
  ...
  methods: {
    ...
    fetchData() {
      this.loaded = false;
      axios
        .get(`${this.$basePrefix}survey_responses/mobilizers`, {
